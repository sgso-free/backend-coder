...
class Contenedor {
    ...
    async deleteAll () {
        try {
            //Save a empty array to the file
            let jData =  JSON.stringify([])  

            const pr = await fs.promises.writeFile(this.pathName,jData,'utf-8')  

            return jData
        } catch(error) {
            console.log('An error ocurre when deleting all products (in deteleAll): ',error.message)
        }
    }
    ...
    async getByRandom () {
        try {
            ...
            //read all object, and finde the id
            let productsRead = await this.getAll(); 
            let amount = productsRead.length;
            console.log("Lenght is:",amount);
            let searchId = Math.floor(Math.random() * (amount-1));
            let prFind = productsRead[searchId]; 
            return prFind
        } catch(error) {
            console.log('An error when get object by id (in getbyId): ',error.message)
            //throw new Error(error.message)
